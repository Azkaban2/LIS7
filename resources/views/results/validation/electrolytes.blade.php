...
<div class="flex">
    ...
    <!-- Electrolytes Validation Content -->
    <div class="ml-64 flex-1 p-6 container mx-auto mt-10">
        <div class="bg-white shadow-md rounded p-6">
            <h3 class="text-lg font-semibold text-center mb-4">Validation - Electrolytes</h3>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    resultInputs.forEach(input => {
        input.addEventListener("input", function () {
            ...
            const gender = this.getAttribute("data-gender").trim().toLowerCase();
            let range = this.getAttribute("data-range").trim();
            let flagSpan = row.querySelector("td:nth-child(5) span");
            ...
            // Validate input value against the selected range
            if (!isNaN(value) && !isNaN(min) && !isNaN(max)) {
                isValid = value >= min && value <= max;
            }

            // Update the flag UI correctly
            if (flagSpan) {
