...
<div class="flex">
    ...
    <div class="ml-64 flex-1 p-6 container mx-auto mt-10">
        <div class="bg-white shadow-md rounded p-6">
            <h3 class="text-lg font-semibold text-center mb-4">Validation - Serology</h3>
...
<table class="w-full border-collapse">
    ...
    <tbody>
        ...
        @foreach($validatedResults as $index => $result)
            @if (!in_array($result['parameter'], ['TSH', 'FT3', 'FT4', 'T3', 'T4', 'CEA', 'CA19-9', 'PSA']))
                <tr class="border-b">
        ...
        <tr class="bg-gray-200">
            <td class="py-2 text-left font-bold" colspan="5">THYROID PANEL</td>
        </tr>
        @foreach($validatedResults as $index => $result)
            @if (in_array($result['parameter'], ['TSH', 'FT3', 'FT4', 'T3', 'T4']))
                <tr class="border-b">
                    <td class="py-2 text-center">{{ $result['parameter'] }}</td>
                    <td class="py-2 text-center">
                        <input type="text" name="validatedResults[{{ $index }}][value]" value="{{ $result['value'] }}" class="w-full text-center border rounded">
        ...
        <tr class="bg-gray-200">
            <td class="py-2 text-left font-bold" colspan="5">TUMOR MARKER</td>
        </tr>
        @foreach($validatedResults as $index => $result)
            @if (in_array($result['parameter'], ['CEA', 'CA19-9', 'PSA']))
                <tr class="border-b">
                    <td class="py-2 text-center">{{ $result['parameter'] }}</td>
                    <td class="py-2 text-center">
                        <input type="text" name="validatedResults[{{ $index }}][value]" value="{{ $result['value'] }}" class="w-full text-center border rounded">
