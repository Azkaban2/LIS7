...
<div class="flex">
        ...
        {{-- end: Sidebar --}}
  <!-- ECG Validation Content -->
  <div class="ml-64 flex-1 p-6 container mx-auto mt-10">
    <div class="bg-white shadow-md rounded p-6">
        <h3 class="text-lg font-semibold text-center mb-4">Validation - Electrocardiogram (ECG)</h3>
            ...
            <div class="flex justify-between">
                ...
                <div class="w-1/2">
                    ...
                    <p class="mb-1"><strong>Birthday:</strong> 
                        {{ isset($patientDetails['birthday']) ? \Carbon\Carbon::parse($patientDetails['birthday'])->format('F j, Y') : '' }}
                    </p>
                ...
                <div class="w-1/2">
                    ...
                    <p class="mb-1"><strong>Date Performed:</strong> 
                        {{ isset($patientDetails['date_performed']) ? \Carbon\Carbon::parse($patientDetails['date_performed'])->format('F j, Y') : '' }}
                    </p>
                    <p class="mb-1"><strong>Date Released:</strong> 
                        {{ isset($patientDetails['date_released']) ? \Carbon\Carbon::parse($patientDetails['date_released'])->format('F j, Y') : '' }}
                    </p>
        ...
        <form action="{{ route('generate-pdf') }}" method="POST" id="validationForm">
            ...
            <table class="w-full border-collapse">
                ...
                <tbody>
                    @foreach($validatedResults as $index => $result)
                        ...
                        {{-- Ensure each input is only added once --}}
            ...
            <div class="mt-4">
                ...
                <textarea id="remarks" name="remarks" rows="4" class="mt-1 block w-full border border-gray-300 rounded p-2">
                    {{ old('remarks', $remarks ?? '') }}
                </textarea>
            </div>
